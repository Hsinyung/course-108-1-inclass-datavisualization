...
graphData$data %>%
  ...
  geom_vline(aes(xintercept = 2009),linetype="dotted") ->graphData$oringinalDesign
```
...
```{r}
element_text(angle = 45) ->elementText
# elementText %>% View
```
...
graphList$經常薪資_x有排序_scalLimits_gmWidth_asp1_col0 +
  theme(
    axis.text.x=element_text(angle=45, hjust=1,vjust = 0.95)  #hjust為水平，vjust為垂直，介於0~1
...
```{r}
ggplot2::cut_interval(x,n=8) -> x_interval #切割連續變數 #x_interval為factor
levels(x_interval)
```
...
graphList$圖書_ggplotOnly +
  stat_count(
    aes(x=學院,y=stat(prop),group="全校"), #算各院的比重，y軸不同
...
graphList$圖書_ggplotOnly +
  stat_count(
    aes(x=學院,y=stat(prop),group="全校"),  ##prop算占比
    geom="bar" # 使用資料 x=學院 進行count計算，並畫bar
  )
```
